...
<div class="container-xl">
	<h1 class="app-page-title">{{ _lang('Account Statement') }}</h1>
	<div class="row">
		<div class="col-12">
					...
					<div class="report-params">
						<form class="validate" method="post" action="{{ url('user/reports/account_statement/view') }}">
							<div class="row">
								...
								<div class="col-md-2 mb-3">
									<button type="submit" class="btn app-btn-primary" style="margin-top: 32px;">{{ _lang('View Report') }}</button>
								</div>
							</form>
								...
								<tr>
									...
									<td>
									    @if($transaction->dr_cr == 'dr')
											<span class="badge bg-danger">{{ _lang('Debit') }}</span>
										@elseif($transaction->dr_cr == 'cr')
											<span class="badge bg-success">{{ _lang('Credit') }}</span>
										@endif
									</td>
									...
									<td class="status">
									   @if($transaction->status == 'pending')
										...
										@elseif($transaction->status == 'reject')
											<span class="badge bg-danger">{{ _lang('Rejected') }}</span>
										@endif
									</td>

									<td class="text-center"><button class="app-btn-secondary btn-sm ajax-modal" data-title="{{ _lang('View Transaction Details') }}" data-href="{{ url('user/view_transaction/' . $transaction->id) }}">{{ _lang('View') }}</button></td>
								</tr>
								@endforeach
							@endif
						</tbody>
