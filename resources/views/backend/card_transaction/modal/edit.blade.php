	  ...
	  <div class="form-group">
		...
		<select class="form-control select2" name="card_id" value="{{ old('card_id') }}" required>
            ...
            @foreach(\App\Card::where('status',1)->get() as $card)
                <option value="{{ $card->id }}" {{ $cardtransaction->card_id == $card->id ? 'selected' : '' }}>{{ $card->card_number.' - '.$card->card_type->card_type }}</option>
            @endforeach
		</select>	
	  </div>
	</div>
	...
	<div class="col-md-12">
	  <div class="form-group">
		...
		<select class="form-control" name="status" required>
            <option value="1" {{ $cardtransaction->status == '1' ? 'selected' : '' }}>{{ _lang('Completed') }}</option>
            <option value="0" {{ $cardtransaction->status == '0' ? 'selected' : '' }}>{{ _lang('Pending') }}</option>
            <option value="2" {{ $cardtransaction->status == '2' ? 'selected' : '' }}>{{ _lang('Rejected') }}</option>
		</select>	
	  </div>
	</div>
